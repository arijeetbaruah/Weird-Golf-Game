		...
		namespace Graphics
		{
			namespace Impl
			{
				...
				public:
					...
					DisplayBuffer* m_hdrDisplayBuffers;
					...
					public:
						...
						int initialize(uint32_t width, uint32_t height, uint32_t pitch,
									   ...
									   void* const *vrBuffers, void* const *hdrBuffers, bool enableVrView, bool enableS3dView, bool enableHdrView );
						int finalize(void);
				...
				public:
					...
					RenderTarget* getRenderTarget(uint32_t bufferIndex, GraphicsContext::FrameBufferSide side)
					{
						...
						if( m_videoInfo.isHdrDisplaymode() ){
							return &m_hdrDisplayBuffers[bufferIndex].m_renderTargetLeft;
						}
					...
					int initialize(Impl::GraphicsLoaderImpl *loader, uint32_t width, uint32_t height, uint32_t numBuffers,
								   ...
								   bool isVrView, bool isStereoView, bool isHdrView);
					int finalize(void);
				...
				public:
					...
					ShaderRwResource m_vertexShaderRwResources[sce::Gnm::kSlotCountRwResource];
					uint32_t m_vertexShaderRwResourcesUsedMask;
					ShaderRwResource m_fragmentShaderRwResources[sce::Gnm::kSlotCountRwResource];
					uint32_t m_fragmentShaderRwResourcesUsedMask;
					ShaderRwResource m_computeShaderRwResources[sce::Gnm::kSlotCountRwResource];
				...
				public:
					...
					int setRwTexture(ProgramType programType, Texture *texture, uint32_t index);
					...
					int setRwResource(ProgramType programType, Texture *texture, uint32_t index);
				...
				public:
					...
					int setRwResource(ProgramType programType, Texture *texture, uint32_t index)            { return m_bufferState.setRwResource(programType, texture, index); }
...
#pragma region laoder_wrapper
					...
					int createRwTexture(Texture **outTexture,  TextureFormat texFormat,  uint32_t width,  uint32_t height,  uint32_t mipCount) {
						return m_loader.createRwTexture(outTexture, texFormat, width,  height, mipCount);
					}
