...
int ssgi::TextureImpl::setAccessMode(int GPUWrite)
{
	if (GPUWrite) {
		m_texture.setResourceMemoryType(sce::Gnm::ResourceMemoryType::kResourceMemoryTypeSC);
	}
	else {
		m_texture.setResourceMemoryType(sce::Gnm::ResourceMemoryType::kResourceMemoryTypeRO);
	}
	return SCE_OK;
}
