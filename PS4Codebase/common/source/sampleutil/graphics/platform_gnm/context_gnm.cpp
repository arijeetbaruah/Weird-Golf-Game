						  ...
						  uint32_t usedBufferMask)
{
	uint32_t index = 0;
	while(index < 32){
		uint8_t cz = __lzcnt32(usedBufferMask);
...
static inline void referBuffersRw(ssgi::ReferringResources &rr,
	ssgi::GpuBufferState::ShaderRwResource* resources,
						  uint32_t usedBufferMask)
{
	uint32_t index = 0;
	while(index < 32){
						  ...
						  unsigned __int128 usedBufferMask)
{
	...
	uint32_t index = 0;
	while(index < 64){
		uint8_t cz = __lzcnt64(usedBufferMaskHigh);
		//printf("index=%d, usedBufferMask=%#x, cz=%d\n", index, usedBufferMask, cz);
		if(index+cz >= 64){
			break;
		}
	...
	index = 64;
	while(index < 128){
		uint8_t cz = __lzcnt64(usedBufferMaskLow);
		//printf("index=%d, usedBufferMask=%#x, cz=%d\n", index, usedBufferMask, cz);
		if(index+cz >= 128){
			break;
		}
...
int ssgi::GpuBufferState::initialize(void)
{
	...
	for(uint32_t i=0; i<sce::Gnm::kSlotCountRwResource; i++){
		m_vertexShaderRwResources[i].clear();
		m_fragmentShaderRwResources[i].clear();
		m_computeShaderRwResources[i].clear();
	}
...
int ssgi::GpuBufferState::finalize(void)
{
	...
	for(uint32_t i=0; i<sce::Gnm::kSlotCountRwResource; i++){
		if(m_vertexShaderRwResources[i].resource != NULL){
			sce::SampleUtil::Impl::destroyImpl(m_vertexShaderRwResources[i].resource);
			m_vertexShaderRwResources[i].clear();
		}
		if(m_fragmentShaderRwResources[i].resource != NULL){
			sce::SampleUtil::Impl::destroyImpl(m_fragmentShaderRwResources[i].resource);
			m_fragmentShaderRwResources[i].clear();
		}
		if(m_computeShaderRwResources[i].resource != NULL){
			sce::SampleUtil::Impl::destroyImpl(m_computeShaderRwResources[i].resource);
			m_computeShaderRwResources[i].clear();
		}
	}
	...
	for(uint32_t i=0; i<sce::Gnm::kSlotCountConstantBuffer; i++){
		...
		if(m_computeUniformBuffers[i] != NULL){
			sce::SampleUtil::Impl::destroyImpl(m_computeUniformBuffers[i]);
			m_computeUniformBuffers[i] = NULL;
		}
	}
...
int ssgi::GpuBufferState::setRwTexture(ProgramType programType, Texture *texture, uint32_t index)
{
	...
	if(r[index].resource != NULL){
		...
		*usedMask = setUsedBit(*usedMask, index);
		...
		m_gfxc->setRwTextures(stage, index, 1, NULL);
		*usedMask = setUsedBit(*usedMask, index);
	}
...
int ssgi::GpuBufferState::setRwResource(ssg::ProgramType programType, ssg::Texture *texture, uint32_t index)
{
	return setRwTexture(programType, texture, index);
}
...
int ssgi::GraphicsContextImpl::initialize(uint32_t width, uint32_t height, GraphicsContextOption* option )
{
	...
	ret = m_frameBuffer->initialize(&m_loader, width, height, m_graphicsOption.numFrameBuffer,
		stencil,htile,	m_graphicsOption.multisampleMode, m_graphicsOption.enableVrView, m_graphicsOption.enableS3dView, m_graphicsOption.enableHdrView );
	if(ret < 0){
...
void ssgi::GraphicsContextImpl::setUpDrawBuffers(void)
{
	...
	*/
	...
	// Vertex RwResource
	stage = getShaderStage(kProgramTypeVertex);
	GpuBufferState::ShaderRwResource *rw = m_bufferState.m_vertexShaderRwResources;
	index = 0;
	...
	while(index < 32){
		...
		SCE_SAMPLE_UTIL_ASSERT(rw[index+cz].resource != NULL);
		if (rw[index+cz].buffer != NULL) {
			m_currentGfxc->setRwBuffers(stage, index+cz, 1, rw[index+cz].buffer);
		} else {
			m_currentGfxc->setRwTextures(stage, index+cz, 1, rw[index+cz].texture);
		}
...
void ssgi::GraphicsContextImpl::setUpComputeBuffers(void)
{
	...
	*/
	// Compute RwResource
	GpuBufferState::ShaderRwResource *rw = m_bufferState.m_computeShaderRwResources;
	index = 0;
