...
bool ssgi::FrameBuffer::VideoInfo::isDisplaymodeSupported(ssg::GraphicsContext::DisplayMode mode)
{
	...
	if(mode == GraphicsContext::kDisplayModeS3dView){
		...
		return false;
	}else if(mode == GraphicsContext::kDisplayModeHdrView){
		if (info.capability & SCE_VIDEO_OUT_DEVICE_CAPABILITY_BT2020_PQ) {
			return true;
	    }
		return false;
	}
...
int ssgi::FrameBuffer::VideoInfo::setDisplaymode(ssg::GraphicsContext::DisplayMode m)
{
	...
	if(m == GraphicsContext::kDisplayModeS3dView){
		...
		m_displayMode = m;
		return SCE_OK;
	}else if(m == GraphicsContext::kDisplayModeHdrView){
		SceVideoOutMode mode;
		sceVideoOutModeSetAny(&mode);
		
		mode.colorimetry = SCE_VIDEO_OUT_COLORIMETRY_BT2020_PQ;
...
int ssgi::FrameBuffer::VideoInfo::initialize(uint32_t width, uint32_t height, uint32_t pitch, 
										   ...
										   void* const * hdrBuffers,
										   bool enableVrView, bool enableS3dView, bool enableHdrView
										   )
{
	int ret;
	...
	if(m_enableS3dView){
		...
		ret= sceVideoOutRegisterStereoBuffers(m_handle, bufferStart, buffers, numBuffers, &attribute );
		SCE_SAMPLE_UTIL_ASSERT(ret >= 0);
		m_S3dBufferIndexStart = bufferStart;
		bufferStart	+= numBuffers;
	}
	...
	if(m_enableVrView){
		...
		ret= sceVideoOutRegisterBuffers(m_handle, bufferStart, vrBuffers, numBuffers, &attribute );
		SCE_SAMPLE_UTIL_ASSERT(ret >= 0);
		m_VrBufferIndexStart = bufferStart;
		bufferStart	+= numBuffers;
	}
	...
	if(enableHdrView){
		SceVideoOutBufferAttribute attribute;
		sceVideoOutSetBufferAttribute(&attribute,
			SCE_VIDEO_OUT_PIXEL_FORMAT_B10_G10_R10_A2_BT2020_PQ,
#ifdef TILED
			SCE_VIDEO_OUT_TILING_MODE_TILE,
#else
			SCE_VIDEO_OUT_TILING_MODE_LINEAR,
		...
		ret= sceVideoOutRegisterBuffers(m_handle, bufferStart, hdrBuffers, numBuffers, &attribute );
		SCE_SAMPLE_UTIL_ASSERT(ret >= 0);
		m_HdrBufferIndexStart = bufferStart;
		bufferStart	+= numBuffers;
	}
...
void ssgi::FrameBuffer::VideoInfo::submitFlip(sce::Gnmx::GfxContext *gfxc, uint32_t bufferIndex)
{
	...
	}else if (m_displayMode == GraphicsContext::kDisplayModeHdrView){
		index = bufferIndex + m_HdrBufferIndexStart;
	}else {
	...
	int32_t	ret	= gfxc->submitAndFlip(m_handle, 
...
void ssgi::FrameBuffer::VideoInfo::waitUntilSafeForRendering(sce::Gnmx::GfxContext *gfxc, uint32_t bufferIndex)
{
	...
	if (m_displayMode == GraphicsContext::kDisplayModeHdrView)
	{
		index = bufferIndex + m_HdrBufferIndexStart;
	} else {
		index = (m_useFirstNormalBuffer ? m_firstNormalBufferIndex : m_secondNormalBufferIndex )
...
int ssgi::FrameBuffer::initialize(GraphicsLoaderImpl *loader,
	...
	bool isVrView, bool isS3dView, bool isHdrView )
{
	int ret;
	m_loader = loader;
	m_width = width;
	...
	void **videoHdrBuffers = (void**)alloca(sizeof(void*) * numBuffers);
	...
	m_hdrDisplayBuffers = new DisplayBuffer[m_numBuffers];
	if (isHdrView)
	{
		for (uint32_t buffer = 0; buffer < m_numBuffers; ++buffer)
		{
			ret = m_hdrDisplayBuffers[buffer].m_renderTargetLeft.initialize(loader, width, height,
				sce::Gnm::kDataFormatB10G10R10A2Unorm,
				m_tileMode,
				sce::Gnm::kNumSamples1, sce::Gnm::kNumFragments1,
				m_enableCmask, m_enableFmask);
			if (ret != SCE_OK){
			...
			videoHdrBuffers[buffer] = (void*)m_hdrDisplayBuffers[buffer].m_renderTargetLeft.m_renderTarget.getBaseAddress();
		}
...
int ssgi::FrameBuffer::finalize(void)
{
	...
	if (m_hdrDisplayBuffers) {
		for (uint32_t buffer = 0; buffer < m_numBuffers; ++buffer)	{
			m_hdrDisplayBuffers[buffer].m_renderTargetLeft.finalize();
			m_hdrDisplayBuffers[buffer].m_renderTargetRight.finalize();
		}
		delete[] m_hdrDisplayBuffers;
		m_hdrDisplayBuffers = NULL;
	}
