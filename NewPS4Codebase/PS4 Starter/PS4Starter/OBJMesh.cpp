...
bool OBJMesh::LoadOBJMesh(std::string filename)	
{
	...
	*/
	std::vector<NCL::Maths::Vector2> inputTexCoords;
	std::vector<NCL::Maths::Vector3> inputVertices;
	std::vector<NCL::Maths::Vector4> inputNormals;
	...
	*/
	...
	while(!f.eof()) {
		...
		if(currentLine == OBJCOMMENT) {		//This line is a comment, ignore it
			...
			currentMesh = new OBJSubMesh();
			inputSubMeshes.emplace_back(currentMesh);

			currentMesh->mtlSrc = currentMtlLib;
			currentMesh->mtlType = currentMtlType;
		}
		else if(currentLine == OBJVERT) {	//This line is a vertex
			auto vertex = std::make_unique<float[]>(3);
			f >> vertex[0]; 
			f >> vertex[1]; 
			f >> vertex[2];
			vertex[0] *= 0.05f;
			vertex[1] *= 0.05f;
			vertex[2] *= 0.05f;
			inputVertices.emplace_back(NCL::Maths::Vector3(vertex[0], vertex[1], vertex[2]));
		}
		else if(currentLine == OBJNORM) {	//This line is a Normal!
			auto normal = std::make_unique<float[]>(3);
			f >> normal[0];
			f >> normal[1]; 
			f >> normal[2];
			inputNormals.emplace_back(NCL::Maths::Vector3(normal[0], normal[1], normal[2]));
		}
		else if(currentLine == OBJTEX) {	//This line is a texture coordinate!
			auto texCoord = std::make_unique<float[]>(2);
			f >> texCoord[0];
			f >> texCoord[1];
			/*
			TODO! Some OBJ files might have 3D tex coords...
			*/
			inputTexCoords.push_back(NCL::Maths::Vector2(texCoord[0], texCoord[1]));
		}
	...
	std::map <string, MTLInfo> materials;
		...
		*/
		if(sm->vertIndices.empty()) {
			...
			for(unsigned int j = 0; j < sm->vertIndices.size(); ++j) {
				m->vertices.push_back(inputVertices[sm->vertIndices[j] - 1]);
			}
			...
			if(!sm->texIndices.empty())	
			{
				...
				for(unsigned int j = 0; j < sm->texIndices.size(); ++j) {
					m->textureCoords.push_back(inputTexCoords[sm->texIndices[j] - 1]);
				}
			}
...
//void	OBJMesh::SetTexturesFromMTL(string &mtlFile, string &mtlType, std::map <string, MTLInfo>& materials) {
//	if(mtlType.empty() || mtlFile.empty()) {
//		return;
//	}
//
//	std::map <string, MTLInfo>::iterator i = materials.find(mtlType);
//
//	if(i != materials.end())
//	{
